...
import { StyleSheet, View, Text, FlatList, TouchableOpacity, Share, Modal, SafeAreaView, Alert } from 'react-native';
import { HistoryService, HistoryItem } from '../services/HistoryService';
import { Colors, Spacing, Radius } from '../theme';
import { ChevronLeft, Trash2, Share2, Barcode, QrCode, X, Copy, FileText } from 'lucide-react-native';
...
export const HistoryView: React.FC<HistoryViewProps> = ({ onBack }) => {
  ...
  const handleExport = async () => {
    if (history.length === 0) {
      Alert.alert("Empty History", "No scans to export.");
      return;
    }
    try {
      await HistoryService.exportToCSV();
    } catch (error) {
      Alert.alert("Export Failed", "Could not export history to CSV.");
    }
  };
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <View style={styles.headerRight}>
          ...
          <TouchableOpacity style={[styles.headerButton, { marginLeft: Spacing.space2 }]} onPress={clearHistory}>
